<?php
// --- databázové funkce --- DODELAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAT

// globální objekt - připojí databázi - vitální!
$_db_ = mysqli_connect("database", "admin", "heslo", "project") or die;
...
function create_uzivatel(string $jmeno, string $heslo) : bool {
    
    $jmeno = dbEscape($jmeno);
    $heslo = dbEscape($heslo);

    if($resut = dbQuery("insert into uzivatele(jmeno, heslo) values($jmeno, $heslo)")){
        return true;
    }
}
